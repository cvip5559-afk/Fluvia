...
struct VoiceAssessmentView: View {
    @ObservedObject var audioRecorder: AudioRecorderManager // 📍 استقبال مدير التسجيل
    var onBack: (() -> Void)? = nil
                    ...
                    ScrollView(showsIndicators: false) {
                        VStack(spacing: 0) {
                            ...
                            HStack(alignment: .center, spacing: 35) {
                                ...
                                // Restart Button (إيقاف التسجيل والرجوع للشاشة الأولى)
                                Button(action: {
                                ...
                                // 🛑 Stop Button (إيقاف التسجيل والبدء بالتحليل مع الـ AI)
                                Button(action: {
                                    audioRecorder.stopRecording()
                                    Task {
                                ...
                                // ⏸️ Pause Button (الانتقال لشاشة التوقف VoiceRecordingPausedView)
                                NavigationLink(destination: VoiceRecordingPausedView(audioRecorder: audioRecorder)) {
                                    VStack(spacing: 6) {
                                        ZStack {
                                            Circle()
                                                .fill(VoiceAssessmentColors.controlButtonBg)
...
// MARK: - Preview
#Preview {
    VoiceAssessmentView(audioRecorder: AudioRecorderManager())
}
